 ...
 * process over stdio, connects an MCP Client, and asserts the 3 tools are
...
test("smoke: server boots over stdio and lists all 3 tools without a token", async () => {
  ...
  try {
    ...
    for (const name of ["get_overdue_tasks", "get_projects", "apply_changes"]) {
